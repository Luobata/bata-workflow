...
```bash
pnpm --dir "/Users/bytedance/luobata/bata-skill/harness" orchestrate --adapter=coco-cli --timeoutMs=30000 --maxConcurrency=2 "用一句话说明为什么测试重要"
```
...
```bash
pnpm --dir "/Users/bytedance/luobata/bata-skill/harness" resume --runDirectory="/绝对路径/.harness/state/runs/<run-id>" --adapter=dry-run
```

### 3.6 指定旧报告继续恢复

```bash
pnpm --dir "/Users/bytedance/luobata/bata-skill/harness" resume --reportPath="/绝对路径/report.json" --adapter=dry-run
```
