...
import { buildHarnessSnapshots } from './harness-live';

const tempRoots: string[] = [];

const createTempStateRoot = () => {
  const root = mkdtempSync(resolve(tmpdir(), 'monitor-harness-live-'));
  tempRoots.push(root);
  return root;
};
...
describe('buildHarnessSnapshots', () => {
  it('falls back to a lead-only shell snapshot when monitor session exists but no run has started yet', async () => {
    ...
    const snapshots = await buildHarnessSnapshots(stateRoot);
  ...
  it('returns a shell snapshot for the current monitor session instead of inferring another workspace-matching Coco session', async () => {
    ...
    const repeatedSnapshots = await buildHarnessSnapshots(stateRoot, { cocoSessionsRoot });
  ...
  it('prefers Coco live session data over harness run data when the monitor session is attached to an active Coco session', async () => {
    const stateRoot = createTempStateRoot();
    const cocoSessionsRoot = resolve(createTempStateRoot(), 'coco-sessions');
    ...
    writeJson(resolve(runDirectory, 'queue.json'), {
      goal: 'older harness monitor view',
  ...
  it('keeps a bound Coco session isolated across waiting, live, and disconnected transitions while a sibling session stays active', async () => {
    ...
    const initialSnapshots = await buildHarnessSnapshots(stateRoot, { cocoSessionsRoot });
    ...
    const siblingOnlySnapshots = await buildHarnessSnapshots(stateRoot, { cocoSessionsRoot });
    ...
    const liveSnapshots = await buildHarnessSnapshots(stateRoot, { cocoSessionsRoot });
    ...
    const disconnectedSnapshots = await buildHarnessSnapshots(stateRoot, { cocoSessionsRoot });
  ...
  it('does not release a disconnected session lease if fresh Coco activity resumes before cleanup runs', async () => {
    ...
    const recoveredSnapshots = await buildHarnessSnapshots(stateRoot, { cocoSessionsRoot });
  ...
  it('builds a live monitor snapshot from the newest matching harness run', async () => {
    const stateRoot = createTempStateRoot();
