...
describe('planner and dispatcher', () => {
  ...
  it('支持通过 -dir 与 -target 组合读取目录和文件目标', () => {
    const workspace = mkdtempSync(resolve(tmpdir(), 'bata-workflow-cli-'))
  ...
  it('run 启动时输出运行目录与 watch 提示', () => {
    ...
    expect(result.status).toBe(0)
    expect(result.stderr).toContain('[bata-workflow] runDirectory:')
    expect(result.stderr).toContain('[bata-workflow] queuePath:')
  ...
  it('支持通过 /bata-workflow-team 触发 run，并把 team-size 作为当前 runtime 的并发桥接值', () => {
    const result = spawnSync(
      process.execPath,
      [tsxCliPath, cliPath, '/bata-workflow-team', '--adapter', 'dry-run', '3', '2:model=gpt5.4', '梳理', '登录链路现状'],
      {
        cwd: repoRoot,
        encoding: 'utf8'
      }
    ...
    expect(result.status).toBe(0)
    const runDirectoryMatch = result.stderr.match(/\[bata-workflow\] runDirectory: (.+)/)
  ...
  it('支持在无 slot override 时通过 --team-size 指定 team-size', () => {
    const result = spawnSync(
      process.execPath,
      [tsxCliPath, cliPath, '/bata-workflow-team', '--adapter', 'dry-run', '--team-size', '3', '修复', 'watch', '视图'],
      {
        cwd: repoRoot,
        encoding: 'utf8'
      }
  ...
  it('支持通过 -- 分隔 /bata-workflow-team 的 team-size 与数字开头 goal', () => {
    const result = spawnSync(
      process.execPath,
      [tsxCliPath, cliPath, '/bata-workflow-team', '--adapter', 'dry-run', '3', '--', '2025', 'Q1', 'roadmap'],
      {
        cwd: repoRoot,
        encoding: 'utf8'
      }
  ...
  it('bata-workflow 脚本默认显式使用 coco-auto adapter', () => {
    const packageJson = JSON.parse(readFileSync(resolve(appRoot, 'package.json'), 'utf8')) as {
      scripts?: Record<string, string>
    }

    expect(packageJson.scripts?.['bata-workflow']).toContain('--adapter coco-auto')
  })
})
