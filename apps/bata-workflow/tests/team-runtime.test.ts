...
describe('team runtime control channel', () => {
  it('persists monitor session metadata into queue state for live monitor bridging', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-monitor-'))
  ...
  it('收到 abort-run 控制命令后停止领取新任务，允许 in-flight 自然收口', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-abort-'))
  ...
  it('收到 reroute-task 控制命令后会以新角色重新执行失败任务', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-reroute-'))
    const assignments = createAssignments(['T1', 'T2'])
  ...
  it('slot override 会以 runtime executionTarget 传给 adapter，且 runtime 会覆盖结果元数据', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-slot-route-'))
    const assignments = createAssignments(['T1', 'T2'])
  ...
  it('slot override 会驱动真实 CocoCliAdapter 路由到对应 backend 命令', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-slot-cli-'))
  ...
  it('slot override 下失败结果也会由 runtime 回填 executionTarget 元数据', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-slot-failed-'))
    const assignments = createAssignments(['T1'])
    assignments[0]!.task.maxAttempts = 1
  ...
  it('默认 coco-auto 路径下 slot override + transport=auto 会走 backend fallback 链路', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-auto-fallback-'))
    const assignments = createAssignments(['T1'])
    const binDir = resolve(runDirectory, 'bin')
    mkdirSync(binDir, { recursive: true })
  ...
  it('maxConcurrency 受限时会优先复用最久空闲的 slot worker', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-oldest-idle-'))
  ...
  it('backend 不变时 slot override 仅改 model/profile 也会保留显式 command', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-command-preserve-'))
    const assignments = createAssignments(['T1'])
  ...
  it('runtime event detail 会带上 worker/slot/pane scope', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-scope-'))
    const assignments = createAssignments(['T1'])
  ...
  it('workspaceRoot 会驱动 repo root 级别的 git artifact snapshot', async () => {
    const workspaceRoot = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-workspace-root-'))
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-team-runtime-artifacts-'))
    const repoRootFile = resolve(workspaceRoot, 'docs', 'spec.md')

    mkdirSync(resolve(workspaceRoot, 'apps', 'bata-workflow'), { recursive: true })
    mkdirSync(resolve(workspaceRoot, 'docs'), { recursive: true })
    writeFileSync(repoRootFile, 'before\n', 'utf8')

    runGit(workspaceRoot, ['init'])
    runGit(workspaceRoot, ['add', '.'])
    runGit(workspaceRoot, ['-c', 'user.name=Bata Workflow Test', '-c', 'user.email=bata-workflow@test.invalid', 'commit', '-m', 'init'])
