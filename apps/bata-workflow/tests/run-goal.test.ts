...
describe('runGoal', () => {
  it('完成最小 dry-run 编排闭环', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-run-goal-'))
  ...
  it('失败任务会按 maxAttempts 重试并留下状态轨迹', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-run-goal-retry-'))
  ...
  it('失败策略可触发 fallback role 路由', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-run-goal-fallback-'))
  ...
  it('testing 失败会生成显式 fix/verify loop', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-run-goal-fix-loop-'))
  ...
  it('将上游任务 summary 注入下游角色执行上下文', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-run-goal-deps-'))
