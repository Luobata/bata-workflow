...
describe('runtime event stream', () => {
  it('按顺序写入多条 runtime event 并可完整读回', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-event-stream-'))
  ...
  it('支持从上次 cursor 之后继续读取新事件', async () => {
    const runDirectory = mkdtempSync(resolve(tmpdir(), 'bata-workflow-event-stream-cursor-'))
