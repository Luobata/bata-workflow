...
import { createBataWorkflowRepoPaths } from './repo-paths.js'
...
const DEFAULT_MONITOR_STATE_ROOT_RELATIVE_PATH = ['.bata-workflow', 'state'] as const
...
export function resolveTmuxManagerFallbackSpecifiers(moduleUrl: string = import.meta.url): string[] {
  const { repoRoot } = createBataWorkflowRepoPaths(moduleUrl)
  return [
...
async function defaultCleanupMonitorBoardProcess(pid: number | null): Promise<void> {
  ...
  const tryKill = (targetPid: number, signal: NodeJS.Signals): boolean => {
    try {
  ...
  const isRunning = (targetPid: number): boolean => {
    try {
...
export async function prepareTeamRunSpecWithTmuxBindings(params: {
  ...
  try {
    ...
    const sessionName = buildSanitizedTmuxName(`bata-workflow-${runLabel}`, tmuxManager.sanitizeName, 'bata-workflow-team')
    ...
    process.stderr.write(`[bata-workflow] tmux bootstrap skipped: ${message}\n`)
    return input
  }
}
    ...
    if (!cleanupPromise) {
      cleanupPromise = (async () => {
        try {
          ...
          process.stderr.write(`[bata-workflow] monitor cleanup skipped: ${message}\n`)
        }
      })()
    }
