...
import { isCredibleCompletedSummary } from '../runtime/coco-adapter.js'
...
export function verifyRun(report: RunReport): VerificationSummary {
  ...
  const completedSummariesValid = report.results
    .filter((result) => result.status === 'completed')
    .every((result) => isCredibleCompletedSummary(result.summary))
  checks.push(completedSummariesValid ? 'completed 摘要有效' : '存在疑似截断或无效的 completed 摘要')

  return {
    ok: completedAll && runtimeSettled && noDanglingClaims && completedSummariesValid,
    checks
  }
}
